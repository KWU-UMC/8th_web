...
- 반환값에 타입을 붙이면 그것이 **`TypeScript`** 🍠✅
  ...
  기본적으로 변수 이름 바로 뒤에 콜론과 함께 타입을 표기합니다.
    ...
    - **null과 undefined의 차이 점에 대해 직접 작성해주세요!** 🍠✅
      ...
      `null`은 ‘값이 없음’을 표현하는 ‘값’이고 `undefined`는 실제로 값이 없음을 표현한다.
    ...
    - 실습 정리 🍠✅
        ...
        모든 타입에 대해서 다른 타입은 물론 `null`도 넣을 수 없었다(null 타입 제외). 래퍼 클래스가 있는 타입의 경우에는 new 키워드로 객체 생성을 하면 object 타입으로 다른 타입이 되기 때문에 마찬가지로 대입할 수 없었다.
        
        - string
            
            ![image.png](attachment:fccf2a2f-116d-4f9f-957a-e0aed7d749b6:image.png)
            ...
            const str5: string = {};
            const str6: string = new String('hello');
            const str7: string = String('hello');
            ```
            
        - number
            
            ![image.png](attachment:8538cf57-861e-4d42-aec8-645db79dae77:image.png)
            ...
            const num4: number = {};
            const num5: number = Number(123);
            const num6: number = new Number(123);
            ```
            
        - boolean
            
            ![image.png](attachment:20182347-018b-41e0-9e65-0a1b56f31d4b:image.png)
            
            ```tsx
            const bool1: boolean = true;
            const bool2: boolean = null;
            const bool3: boolean = 123;
            const bool4: boolean = {};
            const bool5: boolean = Boolean(true);
            const bool6: boolean = new Boolean(true);
            ```
            
        - null
            
            ![image.png](attachment:c414affb-11b4-4ff5-b5e4-65a8ee92e3be:image.png)
            
            ```tsx
            const null1: null = null;
            const null2: null = false;
            const null3: null = 0;
            const null4: null = {};
        ...
        - undefined
            ...
            ![image.png](attachment:2bc477df-b4cf-442a-9834-ba995641feef:image.png)
            
            ```tsx
            const undefined1: undefined = undefined;
            const undefined2: undefined = false;
            const undefined3: undefined = 0;
            const undefined4: undefined = {};
            const undefined5: undefined = null;
            ```
            
        - symbol
            
            ![image.png](attachment:ad8c6cde-12d4-40d0-aeaf-33d343dc5357:image.png)
            
            ```tsx
            const symbol1: symbol = Symbol('symbol1');
            const symbol2: symbol = Symbol();
            const symbol3: symbol = new Symbol();
            const symbol4: unique symbol = Symbol('symbol4');
            const symbol5: symbol = 123;
            const symbol6: symbol = 'hello';
            ```
            
            unique symbol은 symbol과 다르게 컴파일 타임에 유일함을 보장할 수 있다. `const`에만 선언이 가능하게 함으로써 다른 값으로 변경이 불가능하고, unique symbol끼리는 비교 연산이 무조건 false가 된다는 경고를 표시한다.
            
        - bigint
            
            ![image.png](attachment:c6183a86-a431-4d05-a4cd-8d50f2b47a1e:image.png)
            
            ```tsx
            const bigint1: bigint = 123n;
            const bigint2: bigint = null;
            const bigint3: bigint = 123;
            const bigint4: bigint = {};
            const bigint5: bigint = BigInt(123);
            const bigint6: bigint = new BigInt(123);
            const bigint7: bigint = BigInt('123');
            ```
            
        - object
            
            ![image.png](attachment:ae2bdbf2-2850-4dde-8e3a-fe0226191c31:image.png)
            
            ```tsx
            const object1: object = {};
            const object2: object = null;
            const object3: object = 123;
            const object4: object = 'hello';
            const object5: object = String('hello');
            const object6: object = new String('hello');
            ```
...
- 함수에서의 **`TypeScript`** 🍠✅
  ...
  **`parameter`**(매개 변수) 타입은, 매개변수 바로 뒤에 표기하고, 반환값의 타입은, 파라미터 뒤에 콜론과 함께 예상되는 반환값의 타입을 명시해줍니다.
    ...
    - 함수 선언식의 특징에 대해 정리해주세요! 🍠✅
        - 호이스팅 된다. (앞서 언급한 value-hoisting)
        - 스스로의 this를 가진다. 부모의 this를 하려면 보통 캡처를 하는 편인 듯.
...
- 타입 스크립트에만 존재하는 타입 🍠✅
    - any 🍠✅
      ...
      모든 타입스크립트 타입의 부모이다. `null`도 마찬가지로 any 타입의 변수에 대입될 수 있다. 이 타입에 대해서는 모든 작업을 수행할 수 있다. 때문에 사용시 주의가 필요하다.

        ```tsx
        let a: any = null;
        let b: string = a; // 컴파일 오류가 발생하지 않음! b에 대한 전제 조건이 깨질 수 있음
        ```

    - unknown 🍠✅

      어떤 타입인지 모를 때 사용할 수 있다. `any`와 `unknown` 중 가능하면 `unknown`이 권장된다. 이 타입은 사용하기 위해서 타입 체크를 필요로 하기 때문이다.

        ```tsx
        let a: unknown = null;
        let b: string = a; // 컴파일 에러
        ```

    - void 🍠✅

      타입이 없음을 명시한다. 예를 들어 아래는 void 반환 타입에서 string을 반환하고 있으므로 컴파일 에러가 발생한다.
      ...
      단 타입 체크가 완전한 건 아니어 보인다. 예를 들어 아래 코드는 정상적으로 동작한다. `func` 호출 부분에서 린트는 표시될 지언정 `"hello"`가 출력된다.
    ...
    - never 🍠✅
      ...
      모든 타입스크립트 타입의 자식 타입이다. 있어보이는 용어로 “bottom type” 혹은 “universal subtype”이라고도 한다.

      기본적으로 생성될 수 없는 타입이기 때문에(→ 모든 타입의 서브타입인 타입은 만들 수 없기 때문) 무조건 오류가 발생하는 함수의  반환 타입이나 절대로 반환하지 않을 함수의 타입으로 사용되는 경우가 많다.
